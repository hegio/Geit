...
class Spider(BaseSpider):
    ...
    def _throttle(self):
        """限速: 站点风控严格, 请求间隔保持 2s+, 避免触发限流"""
        now = time.time()
        gap = now - self._last_ts
        if gap < 2.0:
            time.sleep(2.0 - gap + random.uniform(0, 0.6))
    ...
    def _sync_cookie(self, resp):
        """记录最近一次成功响应的站点 Cookie, 供播放请求携带 (videocdn CDN 校验需要)"""
        try:
            if resp and getattr(resp, 'cookies', None):
                items = ['%s=%s' % (k, v) for k, v in resp.cookies.items() if k and v]
                if items:
                    self._cookie_str = '; '.join(items)
        except Exception:
            pass

    def _get_requests(self, url, referer, use_proxy):
        """requests 抓取单次尝试. 返回 (ok, text)"""
        try:
            kwargs = {'headers': self._get_headers(referer), 'timeout': 30, 'verify': False}
            if use_proxy and self._proxy_ok:
            ...
            if not self._is_blocked(text):
                self._sync_cookie(r)
                return True, text
            self._log('requests%s 疑似风控页 [%s] %s' % ('代理' if use_proxy else '直连', r.status_code, url))
            return False, text
        except Exception as e:
            self._log('requests%s 请求异常 %s: %s' % ('代理' if use_proxy else '直连', url, e))
            return False, ''

    def _fetch(self, url, referer=None, retries=2):
        """带重试抓取页面. 该站对完整浏览器头敏感; 通道顺序: requests代理 -> requests直连 -> curl代理. 返回 html 字符串"""
        text = ''
        for i in range(retries + 1):
            self._throttle()
            # 1) requests 代理
            ok, text = self._get_requests(url, referer, True)
            if ok:
                self._proxy_fail_count = 0
                return text
            ...
            # 2) requests 直连
            ok, text = self._get_requests(url, referer, False)
            if ok:
                return text
            # 3) curl_cffi (代理) 兜底
            if _USE_CURL:
                try:
                    ...
                    if not self._is_blocked(text):
                        ...
                        self._proxy_fail_count = 0
                        return text
                    self._log('curl 疑似风控页 [%s] %s' % (r.status_code, url))
                except Exception as e:
    ...
    def _resolve_signed_url(self, detail_html, content_url, ref_url=None):
        """JSON-LD 的 contentUrl 为无 token 裸地址, CDN 直接访问 403.
        需从详情页 iframe 播放页 (/Player/V2?payload=...) 提取带 token 的签名地址.
        解析失败时回退裸地址. 返回 (播放地址, 播放页是否解析成功)"""
        try:
            m = re.search(r'<iframe[^>]+src="(/Player/V2[^"]+)"', detail_html)
            if not m:
                return content_url, False
            p_url = self.host + m.group(1)
            p_html = self._fetch(p_url, referer=ref_url or self.host + '/movie/new')
            if p_html:
                mm = re.search(r'(https://videocdn\.avking\.xyz/[^"\'\s]+(?:m3u8|mp4)[^"\'\s]*)', p_html)
                if mm:
                    return mm.group(1), True
                self._log('播放页未找到签名地址: %s' % p_url)
            else:
                self._log('播放页抓取失败: %s' % p_url)
        except Exception as e:
            self._log('签名地址解析异常: %s' % e)
        return content_url, False
    ...
    def detailContent(self, array):
        ...
        try:
            ...
            if play_url:
                # JSON-LD contentUrl 为裸地址, 需从播放页提取带 token 签名地址 (失败回退裸地址)
                play_url, signed_ok = self._resolve_signed_url(html, play_url, url)
                ep_name = '正片'
        ...
        try:
            ...
            for it in items[:5]:
                try:
                    ...
                    if not m3u8:
                        continue
                    m3u8, _ = self._resolve_signed_url(dhtml, m3u8, durl)
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        # videocdn CDN 校验需要站点 Cookie, 带上最近一次会话 Cookie 提高可播率
        if self._cookie_str:
            header['Cookie'] = self._cookie_str
        return {
            'parse': 0,
            'playUrl': '',
            'url': url,
